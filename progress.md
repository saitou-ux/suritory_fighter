...
2026-03-15
- BGM が小さいという要望に対応し、`src/game.js` の `SoundManager` を拡張。
  - タイトル/選択/試合後に流れる `menu` BGM と、ラウンド導入/戦闘中に流れる `battle` BGM を Web Audio の簡易シーケンサで追加。
  - `masterGain` を `0.22` に上げ、BGM 専用の `bgmGain` を追加して全体よりはっきり聞こえるように調整。
  - `render_game_to_text` の `audio` に `bgmMode`, `bgmActive`, `bgmStep` を追加し、状態確認しやすくした。
- `index.html` の説明文も BGM 前提の内容へ更新。
- 検証:
  - `npm run build` 成功
  - 公式 Playwright クライアントで `output/web-game/bgm-title-check` を生成し、`state-0.json` で `audio.bgmMode: menu`, `bgmActive: true` を確認
  - 補助の Playwright スクリプトで `output/web-game/bgm-battle-check-final` を生成し、`state-0.json` で `audio.bgmMode: battle`, `bgmActive: true` を確認

2026-03-17
- トップの紹介文を依頼どおり差し替え。
  - `index.html` のヒーローテキストを `AIゲーム生成選手権 オリジナル2D格闘ゲームです。 ※このゲームに登場するキャラクターは架空のものであり、実在する人物とは一切関係ありません。` に更新。
- 検証:
  - `npm run build` 成功
  - 公式 Playwright クライアントで `output/web-game/hero-copy-update-check` を生成し、タイトル画面の通常表示を確認
  - 補助の Playwright スクリプトで `output/web-game/hero-copy-update-page/shot-0.png` と `hero-text.txt` を生成し、ページ上の文言差し替えを確認

- `index.html` の「戦闘システム」欄の説明文を依頼どおり差し替え。
  - パンチ・キック・波動拳でのノックアウト、後ろ入力ガード、ガード時のダメージ無効、ジャンプ攻撃としゃがみキックのガード崩しについての説明へ更新。
- 検証:
  - `npm run build` 成功
  - 補助の Playwright スクリプトで `output/web-game/battle-copy-update-page/shot-0.png` と `battle-copy.txt` を生成し、戦闘システム欄の新文言を確認

- トップの紹介文を再調整。
  - `index.html` のヒーローテキストを `AIゲーム生成選手権 完全オリジナル2D格闘ゲームです。 ※このゲームに登場するキャラクターは架空のものであり、実在する人物とは一切関係ありません。` に更新。
- 検証:
  - `npm run build` 成功
  - 補助の Playwright スクリプトで `output/web-game/hero-copy-complete-original/shot-0.png` と `hero-text.txt` を生成し、ページ上の新文言を確認

- `index.html` の右端カードから `現在の段階` 見出しだけを削除。
  - 本文は残し、カードのレイアウトを崩さない形で調整。
- 検証:
  - `npm run build` 成功
  - 補助の Playwright スクリプトで `output/web-game/remove-current-stage-heading/shot-0.png` と `section-text.txt` を生成し、見出しが消えて本文のみ残ることを確認

- Web Audio BGM を、権利に触れない完全オリジナルの「主人公テーマ風」へ差し替え。
  - `src/game.js` の `BGM_PATTERNS` を再構成し、タイトル側は静かな導入、対戦側は低音・主旋律・合いの手・打音を重ねた熱い本編に更新。
  - `playBgmStep()` も拡張し、`subBass`, `pad`, `counter`, `noiseFrequency`, `noiseVolume` を扱えるようにした。
- 検証:
  - `npm run build` 成功
  - 公式 Playwright クライアントで `output/web-game/original-bgm-title-check` を生成し、`audio.bgmMode: menu`, `bgmActive: true` を確認
  - 補助の Playwright スクリプトで `output/web-game/original-bgm-battle-check` を生成し、`audio.bgmMode: battle`, `bgmActive: true` を確認

- 下部右側カードの説明文を削除。
  - `index.html` から該当 `info-card` を削除し、`src/style.css` の `info-grid` を `auto-fit` 化して、2カードでも自然に並ぶように調整。
- 検証:
  - `npm run build` 成功
  - 公式 Playwright クライアントで `output/web-game/remove-stage-copy-official` を生成し、タイトル画面の通常表示を確認
  - 補助の Playwright スクリプトで `output/web-game/remove-stage-copy-page/shot-0.png` と `card-count.txt` を生成し、カード数が `2` になっていることを確認

- BGM をさらに作り直し、単音中心の「ポコポコ音」からシンセ + ドラム中心の構成へ更新。
  - `src/game.js` の `BGM_PATTERNS` を刷新し、`drums`, `chord`, `lead`, `counter`, `subBass` を持つ譜面へ変更。
  - `SoundManager` に `DynamicsCompressorNode` を追加し、Web Audio API の `OscillatorNode`, `BiquadFilterNode`, `StereoPannerNode`, `DynamicsCompressorNode` を使って `kick`, `snare`, `hat`, `crash`, `bass`, `lead`, `chord` を別レイヤーで鳴らすようにした。
  - `audio` 状態に `bgmLayers` を追加し、その時点で鳴っているレイヤーを確認できるようにした。
- 検証:
  - `npm run build` 成功
  - 公式 Playwright クライアントで `output/web-game/synth-drum-title-check` を生成し、`audio.bgmLayers: ["hat","chord","lead"]` を確認
  - 補助の Playwright スクリプトで `output/web-game/synth-drum-battle-check` を生成し、`audio.bgmLayers: ["kick","hat","bass","lead"]` を確認

- BGM が前に出すぎていたため、全体を控えめな音量へ再調整。
  - `src/game.js` の `SoundManager` で `bgmGain` を `1.08 -> 0.7` に下げ、BGM バス全体の音量を抑えた。
  - あわせて `kick`, `snare`, `hat`, `crash` の発音音量を下げ、耳につきやすいドラム成分をおとなしくした。
- 検証:
  - `npm run build` 成功
  - 公式 Playwright クライアントで `output/web-game/bgm-volume-reduced-title` を生成し、`audio.bgmMode: menu`, `bgmActive: true`, `bgmLayers: ["hat","bass","counter"]` を確認
  - 補助の Playwright スクリプトで `output/web-game/bgm-volume-reduced-battle` を生成し、`audio.bgmMode: battle`, `bgmActive: true`, `bgmLayers: ["kick","hat","bass","lead"]` を確認

- BGM をシンセ+ドラム版から、以前のオリジナルBGM構成へ戻した。
  - `src/game.js` の `BGM_PATTERNS` を `pad / lead / counter / subBass` 中心の旧構成に差し替えた。
  - `SoundManager` の BGM 再生処理を旧構成寄りへ戻しつつ、`bgmLayers` は引き続き出力できるよう維持した。
  - BGM 音量は旧版そのままではなく、SE が埋もれにくいよう `bgmGain` を `0.82` に設定した。
- ジャンプ / パンチ / キック / 波動拳の SE を強化。
  - `SoundManager` に `playJumpSe`, `playPunchSe`, `playKickSe`, `playFireballSe` を追加し、音色とノイズ成分を分けて聞き分けやすくした。
  - `play("jump" | "punch" | "kick" | "fireball")` から新しい専用 SE を呼ぶよう更新した。
- 検証:
  - `npm run build` 成功
  - 公式 Playwright クライアントで `output/web-game/bgm-revert-title-check` を生成し、`audio.bgmMode: menu`, `bgmActive: true`, `bgmLayers: ["bass","pad"]` を確認
  - 補助の Playwright スクリプトで `output/web-game/se-sequence-clean` を生成し、対戦画面で `audio.bgmMode: battle`, `bgmActive: true`, `bgmLayers: ["bass","lead","noise"]` を確認
  - 補助の Playwright スクリプトで `output/web-game/se-direct-playback-check` を生成し、`audio.recent: ["jump","punch","kick","fireball"]` を確認

- 「リュウのテーマを流せるか」を相談していた時点の BGM 音量感に合わせて、BGM ゲインを再度調整。
  - `src/game.js` の `SoundManager` で `bgmGain.gain.value` を `0.82 -> 0.92` に戻した。
  - BGM の譜面構成や強化済みの SE は維持しつつ、その時点の鳴り方へ寄せた。
- 検証:
  - `npm run build` 成功
  - 公式 Playwright クライアントで `output/web-game/bgm-ryu-request-title` を生成し、`audio.bgmMode: menu`, `bgmActive: true`, `bgmLayers: ["bass","lead","counter"]` を確認
  - 補助の Playwright スクリプトで `output/web-game/bgm-ryu-request-battle` を生成し、`runtime.json` の `bgmGain: 0.9200000166893005` と `audio.bgmMode: battle`, `bgmActive: true` を確認

- BGM を下げ、ジャンプ / パンチ / キック / 波動拳の SE を前に出すよう再調整。
  - `src/game.js` の `SoundManager` で `bgmGain.gain.value` を `0.92 -> 0.72` に下げた。
  - `playJumpSe`, `playPunchSe`, `playKickSe`, `playFireballSe` の主音とノイズ成分の音量をそれぞれ引き上げ、4種の SE が BGM に埋もれにくいようにした。
- 検証:
  - `npm run build` 成功
  - 公式 Playwright クライアントで `output/web-game/bgm-se-balance-title` を生成し、タイトル画面の通常表示と `audio.bgmMode: menu`, `bgmActive: true` を確認
  - 補助の Playwright スクリプトで `output/web-game/bgm-se-balance-runtime` を生成し、`runtime.json` の `bgmGain: 0.7200000286102295` と `audio.recent: ["jump","punch","kick","fireball"]` を確認
  - 補助の Playwright スクリプトで `output/web-game/bgm-se-balance-battle` を生成し、対戦画面の表示崩れがないことを確認
